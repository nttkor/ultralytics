    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            def _forward(x):
                ...
                return self.forward(x)[0] if isinstance(m, (Segment, Segmentv2, Segmentv3, Segmentv4, Segmentv4_add, Segmentv5, Segmentv6, Segmentv7, Segmentv8, YOLOESegment, Pose, OBB)) else self.forward(x)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, Segmentv2, Segmentv3, Segmentv4, Segmentv4_add, Segmentv5, Segmentv6, Segmentv7, Segmentv8, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect}
        ):
            args.append([ch[x] for x in f])
            if m in {Segment, Segmentv2, Segmentv3, Segmentv4, Segmentv4_add, Segmentv5, Segmentv6, Segmentv7, Segmentv8, YOLOESegment}:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, YOLOEDetect, Segment, Segmentv2, Segmentv3, Segmentv4, Segmentv4_add, Segmentv5, Segmentv6, Segmentv7, Segmentv8, YOLOESegment, Pose, OBB}:
                m.legacy = legacy
